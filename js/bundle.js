...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  //Если пользователь долистал до конца страницы, то вызываем модальное окно
...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
  ...
  next.addEventListener('click', () => {
    ...
    slidesNumber(slides, slideIndex, current);
  ...
  function slidesNumber(slides, slideIndex, current) {
    if (slides.length <10) {
      current.textContent = `0${slideIndex}`;
    } else {
      current.textContent = slideIndex;
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default)('.timer', '2021-11-25');
